...
(function ($) {
            ...
            grecaptcha.execute('6Lct7QMfAAAAAK7LSYqewbZcIlhkQLPxwiEArhtU', { action: 'application_form'}).then(function(token){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        let popup_title = lang == 'en' ? 'Error!' : 'Помилка!';
                        let popup_text = data;

                        if(data == 'success'){
                            let popup_title = lang == 'en' ? 'Congratulations!' : 'Вітаємо!';
                            let popup_text = lang == 'en' ? 'You still need to confirm your email to complete your registration. We have sent you a letter with instructions to this address. Check your mailbox and follow the link in the letter.' : 'Для завершення реєстрації потрібно ще підтвердити вашу електронну пошту. Ми надіслали вам листа з інструкціями на цю адресу. Перевірте вашу поштову скриньку і перейдіть за посиланням, яке міститься в листі.';
                            $('.eecuForm input').each(function(){
                                $(this).val('');
                            });
                            Swal.fire({
                                ...
                                confirmButtonText: lang == 'en' ? 'Ok' : 'Зрозуміло',
            ...
            grecaptcha.execute('6Lct7QMfAAAAAK7LSYqewbZcIlhkQLPxwiEArhtU', { action: 'application_form'}).then(function(token){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if(data == 'success'){
                            let popup_title = lang == 'en' ? 'Congratulations!' : 'Вітаємо!';
                            let popup_text = lang == 'en' ? 'Your message has been sent.' : 'Ваше повідомлення відправлене.';
                            $('.eecuContact input,.eecuContact textarea').each(function(){
                                $(this).val('');
                            });
                        }
    ...
    if ($(".obj-select").length > 0) {
        $(".obj-select").select2({placeholder: lang == 'en' ? 'Add community' : 'Додати громаду', allowClear: 1})
    }
    if ($(".ind-ci").length > 0) {
